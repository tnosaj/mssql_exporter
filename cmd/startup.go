...
func evaluateInputs() (internal.Settings, error) {
	...
	flag.StringVar(&includeMetrics, "i", "exec,filespace,index,locks,memory,performance,replication,schedulers,tasks,waits", "Comma seperated list of metrics to gather. Possible values are [exec,filespace,index,locks,memory,performance,replication,schedulers,tasks,waits]")
